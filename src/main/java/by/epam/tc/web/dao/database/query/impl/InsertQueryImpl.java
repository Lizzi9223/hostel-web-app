    ...
    public String getInsertQuery(String tableName){
        ...
        switch (tableName){
            case Metadata.USERS_TABLE:
                insertQuery = "insert into users(login,password,role_id) values(?,?,?);"; break;
            case Metadata.USER_ROLES_TABLE:
                insertQuery = "insert into user_roles(role_name) values(?);"; break;
            case Metadata.ADMINS_TABLE:
                insertQuery = "insert into admins(user_id,name,photo) values(?,?,?);"; break;
            case Metadata.ALL_CLIENTS_TABLE:
                insertQuery = "insert into all_clients(user_id, last_name, first_name, passport_id, date_of_birth, " +
                        "country, phone_number, email) values(?,?,?,?,?,?,?,?);"; break;
            case Metadata.BLACK_LIST_TABLE:
                insertQuery = "insert into blacklist(client_id, reason, since_date) values(?,?,?);"; break;
            case Metadata.REGULAR_CUSTOMERS_TABLE:
                insertQuery = "insert into regular_customers(client_id, since_date, discount, notes) values(?,?,?,?);"; break;
