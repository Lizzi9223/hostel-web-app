...
public class SelectQueryImpl implements SelectQuery {
	...
	@Override
	public String getSelectQueryDoubleWhere(String tableName, String whereClauseColumn1, String whereClauseColumn2) {
		return String.format("select * from %s where %s = ? and %s = ?;", tableName, whereClauseColumn1,
				whereClauseColumn2);
	}
	...
	public String getInnerJoinSelectQuery(String mainTableName, String joinTableName, String joinFieldName) {
		return String.format("select * from %1$s Inner Join %2$s On %1$s.%3$s=%2$s.%3$s;", mainTableName, joinTableName,
				joinFieldName);
	}
	...
	public String getInnerJoinSelectQueryDoubleWhere(String mainTableName, String joinTableName, String joinFieldName,
		...
		return String.format(
				"select * from %1$s Inner Join %2$s On %1$s.%3$s=%2$s.%3$s " + "where %1$s.%4$s = ? and %1$s.%5$s = ?;",
