...
public class RoomDAOImpl implements RoomDAO {
    ...
    public Room findRoomByNumber(int roomNumber) throws DAOException {
        ...
        try{
            con = connectionPool.takeConnection();
            st = con.prepareStatement(selectQueryProvider.getSelectQueryWhere(Metadata.ROOMS_TABLE, Metadata.RoomsTableColumn.ROOM_NUMBER));
            st.setInt(1, roomNumber);
            rs = st.executeQuery();
    ...
    public void updateImage(int imgId, Image image) throws DAOException{
        ...
        try{
            con = connectionPool.takeConnection();
            st = con.prepareStatement(updateQueryProvider.getUpdateQueryWhere(Metadata.IMAGES_TABLE, Metadata.ImagesTableColumn.IMG_ID));
            st.setString(1,image.getImgPath());
            st.setInt(2,image.getRoomNumber());
            st.setInt(3,imgId);
            st.executeUpdate();
