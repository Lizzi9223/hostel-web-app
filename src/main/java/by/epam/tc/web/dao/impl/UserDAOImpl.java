...
public class UserDAOImpl implements UserDAO {
	...
	public List<Admin> getAllAdmins() throws DAOException {
		...
		try {
			...
			rs = st.executeQuery(selectQueryProvider.getInnerJoinSelectQuery(Metadata.USERS_TABLE,
					Metadata.ADMINS_TABLE, Metadata.UsersTableColumn.USER_ID));
	...
	public List<Client> getAllClientUsers() throws DAOException {
		...
		try {
			...
			st = con.createStatement();
			rs = st.executeQuery(selectQueryProvider.getInnerJoinSelectQuery(Metadata.USERS_TABLE,
					Metadata.ALL_CLIENTS_TABLE, Metadata.UsersTableColumn.USER_ID));
	...
	public List<BlackListClient> getAllBlackListClients() throws DAOException {
		...
		try {
			...
			rs = st.executeQuery(selectQueryProvider.getInnerJoinSelectQuery(Metadata.ALL_CLIENTS_TABLE,
					Metadata.BLACK_LIST_TABLE, Metadata.AllClientsTableColumn.CLIENT_ID));
			...
			while (rs.next()) {
				...
				String phone = rs.getString(Metadata.AllClientsTableColumn.PHONE_NUMBER);
				String email = rs.getString(Metadata.AllClientsTableColumn.EMAIL);
				clients.add(new BlackListClient(clientId, reason, sinceDate, name, surname, passport, birth, country,
						phone, email));
			}
		} catch (ConnectionPoolException | SQLException e) {
			throw new DAOException(e);
	...
	public List<RegularClient> getAllRegularClients() throws DAOException {
		...
		try {
			...
			while (rs.next()) {
				...
				String phone = rs.getString(Metadata.AllClientsTableColumn.PHONE_NUMBER);
				String email = rs.getString(Metadata.AllClientsTableColumn.EMAIL);
				clients.add(new RegularClient(clientId, sinceDate, discount, notes, name, surname, passport, birth,
						country, phone, email));
			}
		} catch (ConnectionPoolException | SQLException e) {
			throw new DAOException(e);
	...
	public int addClient(Client client) throws DAOException {
		...
		try {
			con = connectionPool.takeConnection();
			st = con.prepareStatement(insertQueryProvider.getInsertQuery(Metadata.ALL_CLIENTS_TABLE),
					Statement.RETURN_GENERATED_KEYS);
	...
	public Client findClientByUserId(int id) throws DAOException {
		...
		try {
			...
			st = con.prepareStatement(selectQueryProvider.getInnerJoinSelectQueryWhere(Metadata.ALL_CLIENTS_TABLE,
					Metadata.USERS_TABLE, Metadata.UsersTableColumn.USER_ID, Metadata.USERS_TABLE,
	...
	public Client findClientByLogin(String login) throws DAOException {
		...
		try {
			...
			st = con.prepareStatement(
					selectQueryProvider.getInnerJoinSelectQueryWhere(Metadata.ALL_CLIENTS_TABLE, Metadata.USERS_TABLE,
							Metadata.UsersTableColumn.USER_ID, Metadata.USERS_TABLE, Metadata.UsersTableColumn.LOGIN));
			st.setString(1, login);
			rs = st.executeQuery();
	...
	public BlackListClient findInBlacklistById(int id) throws DAOException {
		...
		try {
			...
			st = con.prepareStatement(selectQueryProvider.getInnerJoinSelectQueryWhere(Metadata.BLACK_LIST_TABLE,
					Metadata.ALL_CLIENTS_TABLE, Metadata.BlackListTableColumn.CLIENT_ID,
					Metadata.BlackListTableColumn.CLIENT_ID));
			st.setInt(1, id);
			rs = st.executeQuery();
			...
			while (rs.next()) {
				...
				String phone = rs.getString(Metadata.AllClientsTableColumn.PHONE_NUMBER);
				String email = rs.getString(Metadata.AllClientsTableColumn.EMAIL);
				client = new BlackListClient(clientId, reason, sinceDate, name, surname, passport, birth, country,
						phone, email);
			}
		} catch (ConnectionPoolException | SQLException e) {
			throw new DAOException(e);
	...
	public RegularClient findInRegularCustomersById(int id) throws DAOException {
		...
		try {
			...
			while (rs.next()) {
				...
				String phone = rs.getString(Metadata.AllClientsTableColumn.PHONE_NUMBER);
				String email = rs.getString(Metadata.AllClientsTableColumn.EMAIL);
				client = new RegularClient(clientId, sinceDate, discount, notes, name, surname, passport, birth,
						country, phone, email);
			}
		} catch (ConnectionPoolException | SQLException e) {
			throw new DAOException(e);
	...
	public void updateUser(int userId, User user) throws DAOException {
		...
		try {
			con = connectionPool.takeConnection();
			st = con.prepareStatement(
					updateQueryProvide.getUpdateQueryWhere(Metadata.USERS_TABLE, Metadata.UsersTableColumn.USER_ID));
			st.setString(1, user.getLogin());
			st.setInt(2, userId);
			st.executeUpdate();
		} catch (ConnectionPoolException | SQLException e) {
	...
	public void updateAdmin(int userId, Admin user) throws DAOException {
		...
		try {
			con = connectionPool.takeConnection();
			st = con.prepareStatement(
					updateQueryProvide.getUpdateQueryWhere(Metadata.ADMINS_TABLE, Metadata.UsersTableColumn.USER_ID));
	...
	public void updateBlacklistClient(int clientId, BlackListClient client) throws DAOException {
		...
		try {
			con = connectionPool.takeConnection();
			st = con.prepareStatement(updateQueryProvide.getUpdateQueryWhere(Metadata.BLACK_LIST_TABLE,
					Metadata.BlackListTableColumn.CLIENT_ID));
	...
	public void updateRegularClient(int clientId, RegularClient client) throws DAOException {
		...
		try {
			con = connectionPool.takeConnection();
			st = con.prepareStatement(updateQueryProvide.getUpdateQueryWhere(Metadata.REGULAR_CUSTOMERS_TABLE,
					Metadata.RegularCustomersTableColumn.CLIENT_ID));
	...
	public int getUserId(String login) throws DAOException {
		...
		try {
			con = connectionPool.takeConnection();
			st = con.prepareStatement(
					selectQueryProvider.getSelectQueryWhere(Metadata.USERS_TABLE, Metadata.UsersTableColumn.LOGIN));
			st.setString(1, login);
			rs = st.executeQuery();
			while (rs.next()) {
	...
	private int getRoleId(Role role) throws DAOException {
		...
		try {
			con = connectionPool.takeConnection();
			st = con.prepareStatement(selectQueryProvider.getSelectQueryWhere(Metadata.USER_ROLES_TABLE,
					Metadata.UserRolesTableColumn.ROLE_NAME));
