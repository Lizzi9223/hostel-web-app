...
public class UpdateQueryImpl implements UpdateQuery {
	...
	public String getUpdateQueryWhere(String tableName, String whereClauseColumnName) {
		...
		case Metadata.ALL_CLIENTS_TABLE:
			params = "last_name=?, first_name=?, passport_id=?, date_of_birth=?, country=?, phone_number=?, email=?";
			break;
		case Metadata.BLACK_LIST_TABLE:
			params = "reason=?, since_date=?";
			break;
		case Metadata.REGULAR_CUSTOMERS_TABLE:
			params = "since_date=?, discount=?, notes=?";
			break;
		case Metadata.ALL_STAYS_TABLE:
			params = "client_id=?, room_number=?, from_date=?, to_date=?, notes=?";
			break;
		case Metadata.ROOMS_TABLE:
			params = "room_number=?, cost=?, capacity=?, gender=?, bathroom=?, notes=?";
			break;
		case Metadata.IMAGES_TABLE:
			params = "img_path=?, room_number=?";
			break;
		case Metadata.BOOKING_TABLE:
			params = "user_id=?, room_number=?, from_date=?, to_date=?, guests_count=?, approved=?, approve_date=?, paid=?";
			break;
		}
