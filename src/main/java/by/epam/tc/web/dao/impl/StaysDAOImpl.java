...
public class StaysDAOImpl implements StaysDAO {
    ...
    public Booking findBookingById(int id) throws DAOException {
        ...
        try{
            con = connectionPool.takeConnection();
            st = con.prepareStatement(selectQueryProvider.getSelectQueryWhere(Metadata.BOOKING_TABLE, Metadata.BookingTableColumn.BOOKING_ID));
            st.setInt(1, id);
            rs = st.executeQuery();
            ...
            while (rs.next()){
                ...
                boolean isPaid = rs.getBoolean(Metadata.BookingTableColumn.PAID);
                booking = new Booking(id,userId,roomNumber,fromDate,toDate,guestsCount,isApproved,approveDate,isPaid);
            }
        }
        catch (ConnectionPoolException | SQLException e){
            throw new DAOException(e);
    ...
    public List<Booking> findAllBookingsByRoomNumber(int roomNumber) throws DAOException {
        ...
        try{
            con = connectionPool.takeConnection();
            st = con.prepareStatement(selectQueryProvider.getSelectQueryWhere(Metadata.BOOKING_TABLE, Metadata.BookingTableColumn.ROOM_NUMBER));
    ...
    public Stay findStayById(int id) throws DAOException {
        ...
        try{
            con = connectionPool.takeConnection();
            st = con.prepareStatement(selectQueryProvider.getSelectQueryWhere(Metadata.ALL_STAYS_TABLE, Metadata.AllStaysTableColumn.STAY_ID));
            st.setInt(1, id);
            rs = st.executeQuery();
    ...
    public List<Stay> findAllStaysByRoomNumber(int roomNumber) throws DAOException {
        ...
        try{
            con = connectionPool.takeConnection();
            st = con.prepareStatement(selectQueryProvider.getSelectQueryWhere(Metadata.ALL_STAYS_TABLE, Metadata.AllStaysTableColumn.ROOM_NUMBER));
            st.setInt(1, roomNumber);
            rs = st.executeQuery();
    ...
    public void updateBooking(int id, Booking booking) throws DAOException {
        ...
        try{
            con = connectionPool.takeConnection();
            st = con.prepareStatement(updateQueryProvider.getUpdateQueryWhere(Metadata.BOOKING_TABLE, Metadata.BookingTableColumn.BOOKING_ID));
            st.setInt(1,booking.getUserId());
            st.setInt(2,booking.getRoomNumber());
            st.setDate(3,java.sql.Date.valueOf(booking.getFromDate()));
            st.setDate(4,java.sql.Date.valueOf(booking.getToDate()));
