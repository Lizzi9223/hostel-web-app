					...
					<h3>
						<c:choose>
							<c:when test="${role eq 'ADMIN' }">
								...
								<input class="submit_button open-popup booking" type="button"
									value="${new_booking_word}" style="margin-left: 20px" />
							</c:when>
							<c:otherwise>
						...
						<c:forEach var="booking" items="${bookings}">
							<tbody>
								<tr class="choose-booking" style="cursor: pointer">
									<td scope="row"><c:if
											test="${role eq 'ADMIN' && empty booking.isApproved()}">
											<c:out value="!!!" />
										</c:if></td>
									<c:if test="${role eq 'ADMIN'}">
									...
									<fmt:parseDate value="${dateToParse}" pattern="yyyy-MM-dd"
									...
									<td><fmt:formatDate value="${parsedDate}"
											pattern="dd.MM.yyyy" /></td>
									<td><fmt:formatNumber value="${booking.getPrice()}"
											type="currency" pattern=".0# BYN" /></td>
									<td><c:out value="${booking.isPaid()}" /></td>
									<td style="visibility: hidden">
										<form class="target">
											<input type="hidden" name="${Utility.COMMAND}"
												value="${CommandName.CHOOSE_BOOKING}" /> <input
												type="hidden" name="${Utility.CHOSEN_BOOKING_ID}"
												value="${booking.getId()}" />
										</form>
									</td>
								</tr>
											...
											<c:when test="${empty chosenBookingIsApproved}">
												<tr>
													<td>
														<form>
															<input type="hidden" name="${Utility.COMMAND}"
																value="${CommandName.APPROVE_BOOKING}" /> <input
																type="hidden" name="${Utility.BOOKING_ID}"
																value="${chosenBookingId}" /> <input type="hidden"
																name="${Utility.APPROVE}" value="true" /> <input
																class="submit_button" type="submit"
																value="${approve_word}" style="margin-right: 20px" />
														</form>
													</td>
												</tr>
												<tr>
													<td>
														<form>
															<input type="hidden" name="${Utility.COMMAND}"
																...
																type="hidden" name="${Utility.BOOKING_ID}"
																value="${chosenBookingId}" /> <input type="hidden"
																name="${Utility.APPROVE}" value="false" /> <input
																class="submit_button" type="submit"
																value="${disapprove_word}" />
														</form>
													</td>
												</tr>
												<tr>
											...
											<c:otherwise>
												<tr>
													<td>
														<form>
															<input type="hidden" name="${Utility.COMMAND}"
																value="${CommandName.GUESTS_ARRIVED}" /> <input
																type="hidden" name="${Utility.BOOKING_ID}"
																value="${chosenBookingId}" /> <input
																class="submit_button" type="submit"
																value="${guests_arrived_word}"
												...
												<tr>
													<td>
														<form>
															<input type="hidden" name="${Utility.COMMAND}"
																value="${CommandName.EDIT_BOOKING}" /> <input
																type="hidden" name="${Utility.BOOKING_ID}"
																value="${chosenBookingId}" /> <input
																class="submit_button" type="submit" value="${edit}"
																style="margin-right: 20px" />
														</form>
													</td>
												</tr>
									...
									<c:otherwise>
										<c:choose>
												...
												<c:if test="${chosenBookingIsPaid eq false}">
													<tr>
														<td><input class="submit_button" type="submit"
															value="${pay}" disabled style="margin-right: 10px" /></td>
													</tr>
												</c:if>
												<tr>
													<td>
											...
											<c:when test="${chosenBookingIsApproved eq false}">
												<tr>
													<td><p>
															<c:out value="${booking_was_rejected_msg}" />
														</p></td>
												</tr>
											</c:when>
											<c:otherwise>
												<tr>
													<td><p>
															<c:out value="${booking_not_approved_yet_msg}" />
														</p></td>
												</tr>
											</c:otherwise>
										</c:choose>
						...
						<div class="popup" style="width: auto; padding: 40px">
							...
							<form>
								...
								<input type="date" id="fromDate" name="${Utility.FROM_DATE}"
									value="${booking.getFromDate()}" onchange="setToDate()"
								...
								<label for="questsNumber"><c:out
									...
									style="margin-right: 40px"> <label for="roomNumber"><c:out
										value="${current_room_word}" />:</label> <input type="radio"
									id="roomNumber" name="${Utility.ROOM_NUMBER}"
									value="${booking.getRoomNumber()}" checked>
								<c:out value="${booking.getRoomNumber()}" />
								<br> <input type="hidden" name="${Utility.BOOKING_ID}"
					...
					<div class="popup">
						...
						<form>
							...
							<input type="date" id="fromDate" name="${Utility.FROM_DATE}"
								...
								type="hidden" name="${Utility.COMMAND}"
								value="${CommandName.ADD_BOOKING}" /> <input
								class="submit_button" type="submit" value="${check}"
					...
					<c:when test="${checkResult eq false}">
						<div class="popup-bg" style="display: block">
							<div class="popup" style="width: auto">
								<a href="Controller?command=GO_TO_BOOKINGS_PAGE"> <img
									src="images/close.png" style="width: 25px">
								</a>
								<p>
						...
						<div class="popup-bg" style="display: block">
							<div class="popup" style="width: auto; padding-top: 30px">
								...
								<form>
									<c:choose>
										<c:when
											test="${not empty editedBookingId and editedBookingId ne ''}">
											<b><c:out value="${new_room_not_select_if_no_need}" />:</b>
											<br>
									...
									<br> <input type="hidden"
										name="${Utility.EDITED_BOOKING_ID}" value="${editedBookingId}" />
									<input type="hidden" name="${Utility.COMMAND}"
