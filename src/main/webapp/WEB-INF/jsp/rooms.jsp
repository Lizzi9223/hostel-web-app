			...
			<div class="search form">
				...
				<form id="search-form">
					...
					<div style="margin-bottom: 15px">
						<b><c:out value="${cost}" />:&#160;&#160;&#160;</b>
					</div>

					<div class="middle">
						<label for="priceFrom"><c:out value="${from}" />:&#160;</label> <input
							id="priceFrom" type="text" readonly class="rangeInput"
						...
						<div class="multi-range-slider" id="priceRange">
							...
							<input type="range" id="price-left" min="${minCost}"
								max="${maxCost}" value="${currentMinCost}"> <input
								type="range" id="price-right" min="${minCost}" max="${maxCost}"
					...
					<div style="margin-bottom: 15px; margin-top: 50px;">
						<b><c:out value="${capacity}" />:&#160;&#160;&#160;</b>
					</div>

					<div class="middle">
						<label for="capacityFrom"><c:out value="${from}" />:&#160;</label>
						<input id="capacityFrom" type="text" readonly class="rangeInput"
						...
						<div class="multi-range-slider" id="capacityRange">
							...
							<input type="range" id="capacity-left" min="${minCapacity}"
								max="${maxCapacity}" value="${currentMinCapacity}"> <input
								type="range" id="capacity-right" min="${minCapacity}"
								max="${maxCapacity}" value="${currentMaxCapacity}">
					...
					<div style="margin-bottom: 15px; margin-top: 50px;">
						<b><c:out value="${gender}" />:&#160;&#160;&#160;</b>
					</div>
					<c:choose>
						<c:when test="${currentGender eq 'м'}">
							<input type="radio" id="genderChoice1"
								name="${Utility.SEARCH_GENDER}" value="м" checked>
							<label for="genderChoice1" style="margin-right: 20px"><c:out
									value="${m}" /></label>
							<input type="radio" id="genderChoice2"
								name="${Utility.SEARCH_GENDER}" value="ж">
							<label for="genderChoice2" style="margin-right: 20px"><c:out
									value="${f}" /></label>
							<input type="radio" id="genderChoice3"
								name="${Utility.SEARCH_GENDER}" value="мж">
							<label for="genderChoice3"><c:out value="${mf}" /></label>
						</c:when>
						<c:when test="${currentGender eq 'ж'}">
							<input type="radio" id="genderChoice1"
								name="${Utility.SEARCH_GENDER}" value="м">
							<label for="genderChoice1" style="margin-right: 20px"><c:out
									value="${m}" /></label>
							<input type="radio" id="genderChoice2"
								name="${Utility.SEARCH_GENDER}" value="ж" checked>
							<label for="genderChoice2" style="margin-right: 20px"><c:out
									value="${f}" /></label>
							<input type="radio" id="genderChoice3"
								name="${Utility.SEARCH_GENDER}" value="мж">
						...
						<c:when test="${currentGender eq 'мж'}">
							...
							<label for="genderChoice2" style="margin-right: 20px"><c:out
									value="${f}" /></label>
							<input type="radio" id="genderChoice3"
								name="${Utility.SEARCH_GENDER}" value="мж" checked>
							<label for="genderChoice3"><c:out value="${mf}" /></label>
						</c:when>
						<c:otherwise>
							<input type="radio" id="genderChoice1"
					...
					<div style="margin-bottom: 15px; margin-top: 50px;">
						<b><c:out value="${private_bathroom}" />:&#160;&#160;&#160;</b>
					</div>
					<c:choose>
						<c:when test="${currentIsBathroom eq true}">
							<input type="checkbox" id="bathroom"
								name="${Utility.SEARCH_BATHROOM}" checked>
						</c:when>
						<c:otherwise>
							<input type="checkbox" id="bathroom"
								name="${Utility.SEARCH_BATHROOM}">
						</c:otherwise>
					</c:choose>
					<label for="bathroom"><c:out value="${yes}" /></label><br> <input
						type="hidden" name="${Utility.COMMAND}"
						value="${CommandName.SEARCH_ROOMS}"> <input
						style="margin-bottom: 15px; margin-top: 50px;"
						class="submit_button" type="submit" value="${search}"> <input
						id="clear-button" style="margin-left: 30px" class="buttons"
					...
					<div class="form rooms">
						...
						<div class="info">
							<c:if test="${sessionScope.role ne 'ADMIN'}">
								<div style="margin-bottom: 30px">
									<form>
										<c:choose>
											<c:when test="${empty sessionScope.role}">
												<input class="buttons open-popup" type="button"
													value="${booking_word}">
											</c:when>
											<c:otherwise>
												<input type="hidden" name="${Utility.COMMAND}"
													value="${CommandName.BOOK_ROOM}" />
												<input type="hidden" name="${Utility.ROOM_NUMBER}"
							...
							<div>
								...
								<c:if
									test="${not empty room.getGender() and room.getGender() ne ''}">
									<div>
										<b><c:out value="${gender}" />: </b>${room.getGender()}</div>
								</c:if>
