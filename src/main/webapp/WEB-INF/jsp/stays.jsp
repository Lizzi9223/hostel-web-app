					...
					<h3>
						<c:choose>
							<c:when test="${role eq 'ADMIN' }">
                	 			${all_stays}
                	 			<input class="submit_button open-popup stay"
									type="button" value="${new_stay}" style="margin-left: 20px" />
							</c:when>
							<c:otherwise>
                	 			${my_stays}
                	 		</c:otherwise>
							...
							<tbody>
								<tr class="choose-stay" style="cursor: pointer">
									...
									<td style="visibility: hidden">
										<form class="target">
											<input type="hidden" name="${Utility.COMMAND}"
												value="${CommandName.CHOOSE_STAY}" /> <input type="hidden"
												name="${Utility.CHOSEN_STAY_ID}" value="${stay.getId()}" />
										</form>
									</td>
								</tr>
							</tbody>
									...
									<c:when test="${role eq 'ADMIN'}">
										<tr>
											<td>
												<form>
													<input type="hidden" name="${Utility.COMMAND}"
														value="${CommandName.EDIT_STAY}" /> <input type="hidden"
										...
										<c:if test="${login eq 'ADMIN'}">
											<tr>
												<td>
													<form>
														<input type="hidden" name="${Utility.COMMAND}"
															value="${CommandName.DELETE_STAY}" /> <input
															type="hidden" name="${Utility.STAY_ID}"
															value="${chosenStayId}" /> <input class="submit_button"
						...
						<div class="popup" style="width: auto; padding: 40px">
							...
							<form>
								...
								<input type="hidden" name="${Utility.STAY_ID}"
									value="${stay.getId()}" /> <input type="hidden"
									name="${Utility.COMMAND}"
									value="${CommandName.EDIT_STAY_CHECK}" /> <br> <input
					...
					<div class="popup">
						...
						<form>
							...
							<input type="date" id="fromDate" name="${Utility.FROM_DATE}"
								...
								value="${CommandName.ADD_STAY}" /> <input class="submit_button"
								type="submit" value="${check}" style="margin-right: 50px" /> <input
					...
					<c:when test="${checkResult eq false}">
						<div class="popup-bg" style="display: block">
							<div class="popup" style="width: auto">
								<a href="Controller?command=GO_TO_STAYS_PAGE"> <img
							...
							<div class="popup" style="width: auto; padding-top: 30px">
								...
								<form>
									...
									<br> <input type="hidden" name="${Utility.COMMAND}"
										value="${CommandName.SUBMIT_STAY}" /> <input
										class="submit_button" type="submit" value="${submit_stay}" />
								</form>
							</div>
						</div>
					</c:when>
							...
							<div class="popup" style="width: auto; padding-top: 30px">
								...
								<form>
									<table>
										...
										<tr>
											<td><b>${notes}:</b></td>
											<td><textarea name="${Utility.NOTES}" rows="5" cols="30"
													readonly><c:out value="${editedNotes}"></c:out></textarea></td>
										</tr>
									</table>
									<br> <input type="hidden" name="${Utility.EDITED_STAY_ID}"
										value="${editedStayId}" /> <input type="hidden"
										name="${Utility.COMMAND}" value="${CommandName.SUBMIT_STAY}" />
